...
1. Launch Codex Bar — it appears in the menu bar
2. Click **+** to add a ChatGPT account via OAuth
3. Click **切换 / Switch** on any account to activate it; Codex Bar will confirm and then restart Codex.app
...
Codex Bar uses the same OAuth client ID as the official Codex desktop app to authenticate with `auth.openai.com`. After login, tokens are stored locally in the app sandbox and the active account's tokens are written to `~/.codex/auth.json` for the Codex CLI/app to consume.
...
Codex Bar 是一个 macOS 状态栏应用，用于管理多个 ChatGPT/Codex 账号，支持一键切换并实时监控额度。
